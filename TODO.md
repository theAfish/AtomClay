...
- [x] **Isolate Crystallography Operations**:
...
## 4. Testing
- [x] **Unit Tests**: Add unit tests (using Vitest/Jest) for:
...
## 5. Performance
- [x] **Memoization**: Review `Viewer.jsx` to ensure `threeRef` updates don't cause unnecessary re-renders.
- [x] **Large System Optimization**: If atom count > 10,000, consider using `THREE.InstancedMesh` for atoms instead of individual Meshes (currently used for bonds, but atoms use individual meshes).
